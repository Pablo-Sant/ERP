...
from schemas.vc_vendedor_schema import VendedorCreate, VendedorResponse

router = APIRouter()


async def get_plano_conta_or_404(id: int, db: AsyncSession):
    query = select(Vendedor).filter(Vendedor.vendedorid == id)
...
async def get_planos_conta(db: AsyncSession = Depends(get_session)):
    result = await db.execute(select(Vendedor))
    return result.scalars().all()


@router.post('/', response_model=VendedorResponse, status_code=201)
async def post_plano_conta(payload: VendedorCreate, db: AsyncSession = Depends(get_session)):
    novo = Vendedor(**payload.model_dump())
    db.add(novo)
...
async def put_plano_conta(id: int, payload: VendedorCreate, db: AsyncSession = Depends(get_session)):
