...
from schemas.financeiro_conciliacoes import FinanceiroConciliacoesCreate, FinanceiroConciliacoesResponse

router = APIRouter()


async def get_plano_conta_or_404(id: int, db: AsyncSession):
    query = select(FinanceiroConciliacoes).filter(FinanceiroConciliacoes.id_conciliacao == id)
...
@router.get('/', response_model=List[FinanceiroConciliacoesResponse])
async def get_planos_conta(db: AsyncSession = Depends(get_session)):
    result = await db.execute(select(FinanceiroConciliacoes))
    return result.scalars().all()


@router.post('/', response_model=FinanceiroConciliacoesResponse, status_code=201)
async def post_plano_conta(payload: FinanceiroConciliacoesCreate, db: AsyncSession = Depends(get_session)):
    novo = FinanceiroConciliacoes(**payload.model_dump())
    db.add(novo)
...
@router.get('/{id}', response_model=FinanceiroConciliacoesResponse)
