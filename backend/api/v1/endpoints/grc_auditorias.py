...
from schemas.grc_auditorias import AuditoriaCreate, AuditoriaResponse

router = APIRouter()


async def get_plano_conta_or_404(id: int, db: AsyncSession):
    query = select(Auditoria).filter(Auditoria.id == id)
...
async def get_planos_conta(db: AsyncSession = Depends(get_session)):
    result = await db.execute(select(Auditoria))
    return result.scalars().all()


@router.post('/', response_model=AuditoriaResponse, status_code=201)
async def post_plano_conta(payload: AuditoriaCreate, db: AsyncSession = Depends(get_session)):
    novo = Auditoria(**payload.model_dump())
    db.add(novo)
...
@router.put('/{id}', response_model=Auditoria)
async def put_plano_conta(id: int, payload: AuditoriaCreate, db: AsyncSession = Depends(get_session)):
