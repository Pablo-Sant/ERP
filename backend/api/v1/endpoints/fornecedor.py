...
from schemas.fornecedor_schema import FornecedorCreate, FornecedorResponse

router = APIRouter()


async def get_plano_conta_or_404(id: int, db: AsyncSession):
    query = select(Fornecedor).filter(Fornecedor.id == id)
...
async def post_plano_conta(payload: FornecedorCreate, db: AsyncSession = Depends(get_session)):
...
async def put_plano_conta(id: int, payload: FornecedorCreate, db: AsyncSession = Depends(get_session)):
